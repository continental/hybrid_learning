...
class EarlyStoppingHandle:
    ...
    *Usage:* Initialize the handle, then apply after each epoch
    ...
    def step(self, loss: float) -> bool:
        ...
        self._curr_best_loss = self._curr_best_loss or loss

        # Was there any improvement according to self.min_delta?
        progress: bool = self._curr_best_loss - loss < self.min_delta

        # Update epoch counter according to progress
        if self._no_major_improvement_since is not None and progress:
        ...
        self._curr_best_loss = loss if loss < self._curr_best_loss else \
            self._curr_best_loss
        if self.verbose:
